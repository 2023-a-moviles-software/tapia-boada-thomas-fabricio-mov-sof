...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // adaptador (Iterables)
        BaseDatosSQLite.tablaTiendas = ESQLiteHelper(this)
        listView = findViewById<ListView>(R.id.lv_tiendas)
        val arreglo = BaseDatosSQLite.tablaTiendas!!.consultarTiendas()
        MainActivity.tiendas = arreglo
        adaptador = ArrayAdapter(
            this, // contexto
            android.R.layout.simple_list_item_1, //layout.xml que se va a usar
            MainActivity.tiendas!!
        )
    ...
    override fun onContextItemSelected(item: MenuItem): Boolean {
        return when (item.itemId) {
            R.id.mi_editar -> {
                ...
                intent.putExtra("idTienda",tiendas!!.get(idItemSeleccionado).idTienda)
